...
% Define the lineNumbers variable from Pandoc metadata
\newcommand{\lineNumbers}{$lineNumbers$}

% Default the lineNumbers variable to 'false' (as a string) if it's not set by Pandoc
\ifx\lineNumbers\undefined
    \def\lineNumbers{false}  % Define as 'false' string if not defined
\fi

% Check if lineNumbers is 'true' (as a string)
\ifx\lineNumbers\true
    \usepackage{lineno}
    \renewcommand\linenumberfont{\tiny\rmfamily\color{gray!70}} % Sets line numbers to 8pt font size
    \linenumbers
\else
